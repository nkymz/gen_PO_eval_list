...
def gen_eval_list():
    ...
    wbpath = (path + "HDN_eval_list.xlsx").replace("\\", "/")
    url_2nd_half = ["01.html", "02.html", "03.html", "04.html"]
    ...
    for s in url_2nd_half:
        ...
        # r.content.replace(b"\xad\x86", b"")
        r.encoding = "euc-jp"
        soup = BeautifulSoup(r.text, 'lxml')

        eval_list = soup.find("table").find_all("tr")

        for i, eval_row in enumerate(eval_list):
            if i == 0:
                continue
            for j, element in enumerate(eval_row.find_all("td")):
                ws.cell(row=xlrow, column=j + 1).value = element.string
            xlrow += 1
